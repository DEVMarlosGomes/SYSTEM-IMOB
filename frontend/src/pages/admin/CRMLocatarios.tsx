...
export default function CRMLocatarios() {
  ...
  return (
    <AppLayout search={search} onSearch={setSearch} searchPlaceholder="Buscar locatarios…">
      <PageHeader eyebrow="CRM" title="Locatarios" description="Acompanhe contratos ativos, valores e status de pagamento por inquilino." />
      {loading ? <LoadingScreen /> : data.length === 0 ? <EmptyState title="Nenhum locatario encontrado" /> : (
              ...
              <tbody>
                {data.map((u) => (
                  <tr key={u.id} className="border-t border-line hover:bg-surface-muted/50">
                    ...
                    <td className="px-4 py-3 text-center text-ink-secondary">{u.contract ? `dia ${u.contract.dia_vencimento}` : '—'}</td>
                    <td className="px-4 py-3 text-center">
                      {u.last_payment ? <StatusBadge variant={variantForPaymentLocatario(u.last_payment.status_locatario)}>{statusLocatarioLabel(u.last_payment.status_locatario)}</StatusBadge> : <span className="text-ink-muted">—</span>}
                    </td>
